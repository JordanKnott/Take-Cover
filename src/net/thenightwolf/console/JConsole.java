 ...
 * 
 * Built using foundation created by Mike, GitHub link 
 * 
 * @author TheNightWolf
 * 
 */
public class JConsole extends JComponent implements HierarchyListener {
	...
	protected InputHandler inputerHandler;

	volatile boolean isAwaitingInput = false;
	
	protected String cachedString = null;
	protected int promptLength = -1;
	...
	public JConsole(int columns, int rows) {
	    ...
	    setFocusable(true);
		startUpdateTimer();
		addKeyListener(new ConsoleKeyListener(this));
		setInputHandler(new TakeCoverShell());
		if (DEFAULT_BLINK_ON) {
			setCursorBlink(true);
		}
		print("TakeCover [Version " + Start.version + "] Engine Shell");
		print("Created by NightWolf.");
		print("");
		prompt(">");
	...
	public void exit()
	{
		...
		Debug.terminalActive = false;
		
	}
	
	public String prompt(String prompt)
	{
		setCursorPos(getCursorX(), getCursorY());
		write(prompt,Color.GREEN,Color.BLACK);
	...
	/**
	 * Sets the input handler whose job is to deal with text whenever the enter key is pressed
	 * 
	 * @param inputHandler
	 */
	public void setInputHandler(InputHandler inputHandler)
	{
		this.inputerHandler = inputHandler;
	}
	...
	private Timer getUpdateTimer() {
		...
		updateTimer = new Timer(50, new UpdateTimerAction());
